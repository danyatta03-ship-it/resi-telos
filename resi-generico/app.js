...
function boot(){
  ...
  if(u){ enterApp(u); } else { $('#view-login').classList.remove('hidden'); $('#view-app').classList.add('hidden'); }
  // logo login = brand logo se impostato
...
function enterApp(user){
  ...
  RG_render.rerender();
  if(ADMIN_WORDS.includes(user.toLowerCase())){
    RG_editor.enable();
  }
}
$('#btnLogin').onclick = ()=>{
